...
export default function PatientDashboard() {
  const { emergencies, getActiveEmergencyDelay, getPatientAppointments, currentPatient, navigateTo } = useApp()
  const { t } = useLanguage()
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold mb-2">
              {t('patientDashboard.welcome') || 'Welcome'}{currentPatient ? `, ${currentPatient.name}` : ` ${t('patientDashboard.welcomeToMediflow') || 'to MediFlow Hospital'}`}
            </h1>
            <p className="text-teal-100">
              {t('patientDashboard.bookAppointments') || 'Book appointments and track your healthcare journey'}
            </p>
          </div>
          {currentPatient && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500 mb-1">
                {t('patientDashboard.avgWaitingTime') || 'Average Waiting Time'}
              </p>
              <p className="text-3xl font-bold text-gray-800">
                {currentWaitingTime} <span className="text-lg font-normal text-gray-500">{t('dashboard.minutes') || 'mins'}</span>
              </p>
              {delay > 0 && (
                <p className="text-xs text-red-500 mt-1">
                  (+{delay} {t('dashboard.minutes') || 'mins'} {t('patientDashboard.dueToEmergency') || 'due to emergency'})
                </p>
              )}
            </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg border-l-4 border-cyan-500">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500 mb-1">
                {t('patientDashboard.doctorsAvailable') || 'Doctors Available'}
              </p>
              <p className="text-3xl font-bold text-gray-800">8</p>
              <p className="text-xs text-green-500 mt-1">{t('patientDashboard.readyForConsultation') || 'Ready for consultation'}</p>
            </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg border-l-4 border-emerald-500">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500 mb-1">
                {t('patientDashboard.appointmentsToday') || 'Appointments Today'}
              </p>
              <p className="text-3xl font-bold text-gray-800">{todayAppointments.length}</p>
              <p className="text-xs text-gray-500 mt-1">
                {todayAppointments.length > 0 ? (t('patientDashboard.scheduled') || 'Scheduled') : (t('patientDashboard.noAppointments') || 'No appointments')}
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg">
          ...
          <div className="grid grid-cols-2 gap-4">
            <button 
              ...
              className="p-4 bg-gradient-to-br from-teal-500 to-teal-600 text-white rounded-xl hover:shadow-lg transition-all flex flex-col items-center gap-2"
            >
            ...
            <button 
              ...
              className="p-4 bg-gradient-to-br from-cyan-500 to-cyan-600 text-white rounded-xl hover:shadow-lg transition-all flex flex-col items-center gap-2"
            >
              <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" />
              </svg>
              <span className="text-sm font-medium">
                {t('patientDashboard.findDoctor') || 'Find Doctor'}
              </span>
            </button>
            <button 
              onClick={() => navigateTo?.('patient-appointments')}
              className="p-4 bg-gradient-to-br from-emerald-500 to-emerald-600 text-white rounded-xl hover:shadow-lg transition-all flex flex-col items-center gap-2"
            >
              <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
                <path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67z" />
              </svg>
              <span className="text-sm font-medium">
                {t('patientDashboard.myAppointments') || 'My Appointments'}
              </span>
            </button>
            <button 
              onClick={() => navigateTo?.('medical-history')}
              className="p-4 bg-gradient-to-br from-purple-500 to-purple-600 text-white rounded-xl hover:shadow-lg transition-all flex flex-col items-center gap-2"
            >
              <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
              ...
              <span className="text-sm font-medium">
                {t('patientDashboard.viewRecords') || 'View Records'}
              </span>
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg">
          <h2 className="text-lg font-bold text-gray-800 mb-4">
            {t('patientDashboard.hospitalStatus') || 'Hospital Status'}
          </h2>
            ...
            <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium text-gray-700">
                  {t('patientDashboard.emergencyWard') || 'Emergency Ward'}
                </span>
              </div>
              <span className={`text-sm font-medium ${activeEmergencies.length > 0 ? 'text-red-600' : 'text-green-600'}`}>
                {activeEmergencies.length > 0 ? (t('patientDashboard.busy') || 'Busy') : (t('patientDashboard.normal') || 'Normal')}
              </span>
            </div>
            <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium text-gray-700">
                  {t('patientDashboard.opdServices') || 'OPD Services'}
                </span>
              </div>
              <span className="text-sm font-medium text-green-600">{t('patientDashboard.open') || 'Open'}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center gap-3">
                <div className="w-3 h-3 rounded-full bg-yellow-500"></div>
                <span className="text-sm font-medium text-gray-700">
                  {t('patientDashboard.pharmacy') || 'Pharmacy'}
                </span>
              </div>
              <span className="text-sm font-medium text-yellow-600">{t('patientDashboard.moderateQueue') || 'Moderate Queue'}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center gap-3">
                <div className="w-3 h-3 rounded-full bg-green-500"></div>
                <span className="text-sm font-medium text-gray-700">
                  {t('patientDashboard.laboratory') || 'Laboratory'}
                </span>
              </div>
              <span className="text-sm font-medium text-green-600">{t('patientDashboard.available') || 'Available'}</span>
            </div>
          </div>
        </div>
      </div>
