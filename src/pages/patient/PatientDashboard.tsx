...
export default function PatientDashboard() {
  const { emergencies, appointments, getActiveEmergencyDelay } = useApp()
  const { t } = useLanguage()

  const activeEmergencies = emergencies.filter((e) => e.status === 'active')
  const delay = getActiveEmergencyDelay()
  const todayAppointments = appointments.filter(
    (apt) => apt.date === '2026-02-22' && (apt.status === 'confirmed' || apt.status === 'delayed')
  )
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-gradient-to-r from-teal-600 to-cyan-600 rounded-2xl p-6 text-white shadow-lg">
        <h1 className="text-2xl font-bold mb-2">
          {t('patientDashboard.welcome') || 'Welcome to MediFlow Hospital'}
        </h1>
        <p className="text-teal-100">
          {t('patientDashboard.subtitle') || 'Book appointments and track your healthcare journey'}
        </p>
      </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h3 className="font-bold text-red-700 text-lg">
                {t('patientDashboard.emergencyAlert') || 'Emergency Alert'}
              </h3>
              <p className="text-red-600 mb-2">
                {t('patientDashboard.emergencyMessage') ||
                  'Due to emergency cases, appointments may be delayed.'}
              </p>
              <div className="bg-red-100 rounded-lg p-3">
                <p className="text-sm text-red-700">
                  <span className="font-semibold">
                    {t('patientDashboard.estimatedDelay') || 'Estimated Delay'}:
                  </span>{' '}
                  {delay} {t('common.minutes') || 'minutes'}
                </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg border-l-4 border-emerald-500">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500 mb-1">
                {t('patientDashboard.yourAppointmentsToday') || 'Appointments Today'}
              </p>
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <button className="p-4 bg-gradient-to-br from-emerald-500 to-emerald-600 text-white rounded-xl hover:shadow-lg transition-all flex flex-col items-center gap-2">
              ...
              <span className="text-sm font-medium">
                {t('patientDashboard.checkWaitTime') || 'Check Wait Time'}
              </span>
            </button>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium text-gray-700">
                  {t('patientDashboard.opd') || 'OPD Services'}
                </span>
              </div>
